...
const AMBIENT_SAMPLE_MS = 450;
const AMBIENT_LERP = 0.08;
...
function scorePixel(r, g, b) {
  ...
  if (s < 0.06) return l * 0.12;
  return s * 1.5 + (1 - Math.abs(l - 0.42)) * 0.7;
}

function sampleVideoColor(v) {
...
function mixWhite(rgb, amount) {
  const t = 1 - amount;
  return {
    r: lerpChannel(255, rgb.r, t),
    g: lerpChannel(255, rgb.g, t),
    b: lerpChannel(255, rgb.b, t),
  };
}
...
export class WorkCanvas {
  constructor(canvas, opts = {}){
    ...
    this.ambient = this.videos.map(() => ({ r: 255, g: 255, b: 255 }));
    this._lastAmbientSample = 0;
  ...
  /* ── ambient colour from the films ─────────────────────────────────── */
  _tickAmbient(now) {
    if (now - this._lastAmbientSample < AMBIENT_SAMPLE_MS) return;
    this._lastAmbientSample = now;
    for (let i = 0; i < this.videos.length; i++) {
      if (!this.playing[i]) continue;
      const next = sampleVideoColor(this.videos[i]);
      if (!next) continue;
      const cur = this.ambient[i];
      this.ambient[i] = {
        r: lerpChannel(cur.r, next.r, 0.35),
        g: lerpChannel(cur.g, next.g, 0.35),
        b: lerpChannel(cur.b, next.b, 0.35),
      };
    }
  }

  _paintAmbientBase(c, focus) {
    const pool = focus.length ? focus : this.ambient;
    let r = 0, g = 0, b = 0, w = 0;
    for (const item of focus) {
      const col = this.ambient[item.i];
      const weight = 1 / (1 + item.d / 280);
      r += col.r * weight;
      g += col.g * weight;
      b += col.b * weight;
      w += weight;
    ...
    c.fillStyle = `rgb(${avg.r},${avg.g},${avg.b})`;
    c.fillRect(0, 0, this.w, this.h);
    document.body.style.backgroundColor = `rgb(${avg.r},${avg.g},${avg.b})`;
  }

  _paintTileGlow(c, i, x, y, strength) {
    const col = this.ambient[i];
    const cx = x + this.tileW / 2;
    const cy = y + this.tileH / 2;
    const outer = Math.max(this.tileW, this.tileH) * (1.55 + strength * 0.45);
    const inner = Math.max(this.tileW, this.tileH) * 0.08;
    const g = c.createRadialGradient(cx, cy, inner, cx, cy, outer);
    const peak = 0.42 + strength * 0.38;
    const mid = 0.16 + strength * 0.14;
    g.addColorStop(0, `rgba(${col.r},${col.g},${col.b},${peak})`);
    g.addColorStop(0.32, `rgba(${col.r},${col.g},${col.b},${mid})`);
  ...
  /* ── pass 1 ───────────────────────────────────────────────────────── */
  drawPlane(dt){
    this._tickAmbient(performance.now());
    ...
    for (let i = i0; i <= i1; i++){
      for (let j = j0; j <= j1; j++){
        ...
        for (const s of SLOTS){
          ...
          glows.push({ i: s.i, x, y, d: dd });
        }
      }
    }

    this._paintAmbientBase(c, glows);
    for (const g of glows) {
      const strength = clamp(1 - g.d / (Math.min(this.w, this.h) * 0.72), 0, 1);
      this._paintTileGlow(c, g.i, g.x, g.y, strength);
    }
      ...
      for (let j = j0; j <= j1; j++){
        ...
        for (const s of SLOTS){
          ...
          if (s.kind === 'work'){
            ...
            cards.push({ ...r, href: CARDS[s.i].href, route: CARDS[s.i].route });
          }
        }
      }
    }
  ...
  paintCard(c, card, x, y){
    ...
    c.font = `600 ${Math.round(size * 0.82)}px "SF Pro Display", Georgia, serif`;
    c.fillStyle = '#FF1493';
    c.fillText('Z', cx, cy - size * 1.35);
